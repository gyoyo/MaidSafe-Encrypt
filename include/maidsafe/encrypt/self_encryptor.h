...
bool SelfEncryptor<Storage>::Write(const char *data,
  ...
  data_map_->content.insert(position, data, length);
...
//  }
//
//  memcpy(data, content.string().data(), content.string().size());
//
//  //try {
...
//    //                                                        iv.get());
//
//    std::string chunk_content(reinterpret_cast<char*>(data), length);
...
//    //                                         crypto::SHA512::DIGESTSIZE);
//    data_map_->chunks[chunk_num].hash.assign(reinterpret_cast<char*>(&data_map_->chunks[chunk_num].pre_hash[0]),
//                                             crypto::SHA512::DIGESTSIZE);
//
//    std::lock_guard<std::mutex> guard(chunk_store_mutex_);
...
bool SelfEncryptor<Storage>::Read(char* data, const uint32_t &length, const uint64_t &position) {
  ...
  memcpy(data, data_map_->content.data() + position, length);
...
bool SelfEncryptor<Storage>::Truncate(const uint64_t &position) {
  SCOPED_PROFILE
  if (position > data_map_->content.size())
    return TruncateUp(position);
  else if (position < data_map_->content.size())
...
bool SelfEncryptor<Storage>::TruncateDown(const uint64_t &position) {
  ...
  data_map_->content.resize(position);
...
bool SelfEncryptor<Storage>::TruncateUp(const uint64_t &position) {
  ...
  data_map_->content.append(position - data_map_->content.size(), 0);
